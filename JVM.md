...
#  目录
  ...
  * [一：JVM基础知识](#一jvm基础知识)
    * [1：什么是JVM？](#1什么是jvm)
    * [2：好处](#2好处)
    * [3：JVM和JDK,JRE的关系(面试)](#3jvm和jdkjre的关系面试)
    * [4：常见的JVM](#4常见的jvm)
    * [5: JVM的学习步骤](#5-jvm的学习步骤)
  * [二：JVM 内存模型](#二jvm-内存模型)
    * [1：程序计数器：Program Counter Register](#1程序计数器program-counter-register)
    * [2：虚拟机栈：线程运行需要的内存空间，线程私有  参数-Xss控制,在1MB左右](#2虚拟机栈线程运行需要的内存空间线程私有--参数-xss控制在1mb左右)
    * [3:本地方法桟:给本地方法提供的内存空间,线程私有的](#3本地方法桟给本地方法提供的内存空间线程私有的)
    * [4:堆 heap:线程共享, -Xmx 参数控制堆空间最大值](#4堆-heap线程共享--xmx-参数控制堆空间最大值)
    * [5:方法区 Method Area  线程共享 -XX:MaxMetaspaceSize控制,默认无上限](#5方法区-method-area--线程共享--xxmaxmetaspacesize控制默认无上限)
    * [6:直接内存](#6直接内存)
    * [7:字节码常用指令](#7字节码常用指令)
    * [8:相关JVM参数](#8相关jvm参数)
...
1.8之后在堆中,Minor GC就可以触发垃圾回收,内存不足就是堆内存不足 (符号表也在堆中)
    java.lang.OutOfMemoryError: GC overhead limit exceeded  ,需要把此参数关闭  -XX:-UseGCOverheadLimit,
    会显示 java.lang.OutOfMemoryError: Java heap space
...
可以使用intern方法，主动将串池中还没有的字符串对象放入串池中
 1.8之后,将这个字符串对象尝试放入串池,如果有则不会放入,没有则把对象放入串池,会把串池中的对象返回
